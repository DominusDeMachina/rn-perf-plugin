...
1. **Check the React Native version.**
   - **≥ 0.79** — already the default in modern RN templates. Inspect generated `build.gradle` to confirm; otherwise skip this skill.
...
`android/app/build.gradle` — the entire change:
...
## References
- Plugin-added guidance retained from earlier RN Perf coverage; the 2026 book discusses Hermes bytecode and memory mapping in app-size analysis, but no longer has a standalone "Disable JS Bundle Compression" chapter.
