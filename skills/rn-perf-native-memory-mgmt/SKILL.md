...
**Rule: match `passRetained` with `takeRetainedValue`, and `passUnretained` with `takeUnretainedValue`. Mismatching crashes.**
...
- **`Unmanaged` rules summary**: retained ↔ retained, unretained ↔ unretained. Mismatched mismatches *crash*.
