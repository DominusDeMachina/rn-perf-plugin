...
**iOS:** `ios/Podfile`, top-level `Gemfile` (defines the CocoaPods version; comes by default with every new React Native project), `ios/Pods/` (gitignored), `ios/*.xcworkspace` (open this, **not** `.xcodeproj`).
...
- Confirm the project contains: `ios/Podfile`, top-level `Gemfile`, `ios/Pods/`, `android/build.gradle`, `android/app/build.gradle`, `android/gradle.properties`, `android/gradlew`.
