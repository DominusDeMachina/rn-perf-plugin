...
## Workflow
1. **Gate first.** Apply this skill only if one of these holds (book p. 202):
...
Split-point conversion (book p. 204):
...
## Edge cases & gotchas
- **Hermes makes this far less compelling.** Don't take on Re.Pack migration just for code-splitting unless one of the gate conditions actually applies (book p. 202).
...
- **Module Federation complexity is high.** Version drift between micro-apps is "challenging to handle on your own" (book p. 205). Zephyr Cloud is the book's recommended integration partner.
...
## References
- Book: "The Ultimate Guide to React Native Optimization" (2026), chapter "Load Code Remotely When Needed", pp. 201–205
