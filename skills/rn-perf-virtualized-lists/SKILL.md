...
3. **Decision tree:**
   ...
   - If the app cannot use FlashList v2 because it is still off the New Architecture, or it needs chat-friendly/bidirectional infinite-scroll behavior, evaluate **`@legendapp/list`**.
...
5. **Keep rows light.** Memoize the row component; never put side effects in the render path; avoid inline objects in `renderItem`. The book is explicit: list items need to stay light and side-effect free.
6. **Measure again** on the same device/scenario; persist before/after Flashlight (Android) or Perf Monitor numbers in the PR.

## Code patterns

`ScrollView` → `FlatList` (book pp. 42–45):
...
Add `getItemLayout` for fixed-height rows (book p. 45):
...
`FlatList` / `FlashList` alternative → `LegendList`:
...
- **Flashlight score:** record before/after; the book's example moved a real list from **25/100 → 78/100** swapping `FlatList` for `FlashList` (p. 48).
...
- **`FlatList` keeps unmounted items in its window buffer.** Uses more memory than `FlashList` and "eventually slows the list down" (book p. 46).
...
- **Legend List v2.x is production-ready, but recycling is opt-in.** Enable `recycleItems` for long lists where performance matters, and avoid render-path side effects or local state that assumes one component instance always maps to the same item.
...
## References
- Book: "The Ultimate Guide to React Native Optimization" (2026), "Higher-Order Specialized Components", pp. 41–50.
